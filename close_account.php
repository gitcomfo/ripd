...
if (isset($_POST['submit']) && ($_GET['action'] == 'new')) {
    ...
    //$file_name1 = $_FILES['scan_document_closed']['name'];
    $extension = end(explode(".", $_FILES['scan_document_closed']['name']));
    $scan_doc_name = $account_number."_". $_FILES['scan_document_closed']['name'];
    $scan_doc_path_temp = "images/scan_doc_closed_postpond/".$scan_doc_name;
    if (($_FILES['scan_document_closed']['size'] < 999999999999) && in_array($extension, $allowedExts)) 
            {
                    move_uploaded_file($_FILES['scan_document_closed']['tmp_name'], $scan_doc_path_temp);
    ...
    //print_r($_FILES);
    //echo "</pre>";
   // echo "file_name1: ".$file_name1." hello world"."scan_document:".$scan_document."File_name: ".$_FILES['scan_document_closed']['name']." scan_doc_name: ".$scan_doc_name."<br/>";
    ...
    if ($row_number_customer_account_row > 0) {
        ...
        if ($check_acr_row_number < 1) {
            ...
            if (mysql_query($sql_insert_acr) && mysql_query($sql_update_cfs_user)) {
                $msg = "You Have Successfully Closed Account: " . $account_number;
            } else {
            ...
            if ($current_account_status == "closed") {
                $msg = $account_number . " একাউন্ট-টি বন্ধ আছে ";
            } else {
...
<script>
    function validateForm()
    {
        ...
        var fup = document.getElementById('scan_document_closed');
...
if ($_GET['action'] == 'new') {
    ...
    <div style="padding-top: 10px;">    
        <div style="padding-left: 110px; width: 58%; float: left"><a href="index.php?apps=CRM"><b>ফিরে যান</b></a></div>
        <div style=""><a href="close_account.php?action=new"> একাউন্ট বন্ধ-করন</a>&nbsp;&nbsp;<a href="<?php echo htmlspecialchars($_SERVER['PHP_SELF']) ?>">বন্ধ একাউন্ট লিস্ট</a></div>
    </div>
    <div>           
        <form name="account_close_form" method="POST" enctype="multipart/form-data" onsubmit="return validateForm()">	
            <table class="formstyle"  style=" width: 78%; ">          
                <tr><th style="text-align: center" colspan="3"><h1>একাউন্ট বন্ধ-করণ</h1>
                </th>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>:</td>
                    <td > <input class="box" type="file" id="scan_document_closed" name="scan_document_closed" style="font-size:10px;"/> </td>
                </tr>
                <tr>                    
            ...
            <table class="formstyle"  style=" width: 78%;" >      
                ...
                $sql_closed_account_list = mysql_query("SELECT * from $dbname.account_close_restart, $dbname.customer_account, $dbname.cfs_user
                                                                        Where Customer_account_idCustomer_account=idCustomer_account AND cfs_user_idUser=idUser
                                                                                AND account_status = 'closed' ORDER BY account_name");
