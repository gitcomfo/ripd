...
if (isset($_POST['submit'])) {
    for($dayLoop =1; $dayLoop<=$monthDayNumber;$dayLoop++){
        ...
        $check_SOnDay_sql = mysql_query("SELECT * from ".$dbname.".special_onday where spN_date = '$date_date' And office_type = 'office'");
        $check_SOnDay_row_number = mysql_num_rows($check_SOnDay_sql);
        //check in Special Off Day Table
        $check_SOffDay_sql = mysql_query("SELECT * from ".$dbname.".special_offday where sp_off_day_date = '$date_date' And office_type = 'office'");
        ...
        if($special_on_day!="" && $check_SOnDay_row_number<1){
          $sql_insert_special_onday = "INSERT into $dbname.special_onday (spN_date, spN_description, spN_insert_date, office_type, cfs_user_idUser, offday_officeStore) 
                                                    VALUES ('$date_date', '$special_on_day', NOW(), 'office', '$user_id', '1')";
        ...
        elseif($check_SOnDay_row_number>0){
            $sql_update_special_onday = "UPDATE ".$dbname.".special_onday SET spN_description='$special_on_day', spN_insert_date=NOW(),cfs_user_idUser='$user_id' where spN_date = '$date_date' And office_type = 'office'";
        ...
        if($special_off_day!="" && $check_SOffDay_row_number<1){
            $sql_insert_special_offday = "INSERT into $dbname.special_offday (sp_off_day_date, sp_off_day_description, sp_off_day_insert_date, office_type, cfs_user_idUser, offday_officeStore) 
                                                    VALUES ('$date_date', '$special_off_day', NOW(), 'office', '$user_id', '1')";
            ...
            $sql_update_special_offday = "UPDATE ".$dbname.".special_offday SET sp_off_day_description='$special_off_day', sp_off_day_insert_date=NOW(),cfs_user_idUser='$user_id' where sp_off_day_date = '$date_date' And office_type = 'office'";
...
<div style="padding-top: 10px;">
    ...
    class Calendar {
        ...
        function makeCalendar($msg,$flag) {            
            echo '<div style="padding-left: 50px;"><a href="index.php?apps=OSP"><b>ফিরে যান</b></a></div>';
            ...
            echo '<td colspan="5" height="40px"><b>অফিস ডে এন্ড অফ ডে ক্যালেন্ডার তৈরি</b></td></tr><tr>';
            if(!empty($msg)){
                if($flag=='true'){
            ...
            for ($i = 1; $i <= NUM_OF_DAYS + START_DAY; $i++) {
                if ($i > START_DAY) {
                    ...
                    $sql_SOnDay=  mysql_query("SELECT * from ".$dbname.".special_onday where spN_date = '$CurDate' And office_type = 'office'");
                    ...
                    $sql_SOffDay=  mysql_query("SELECT * from ".$dbname.".special_offday where sp_off_day_date = '$CurDate' And office_type = 'office'");
